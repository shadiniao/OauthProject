...
public class UserControllerTest {
    ...
    public void whenUpdateSuccess() throws Exception {
        // 明年
        long nextYearsTime = LocalDateTime.now().plusYears(1).atZone(ZoneId.systemDefault()).toInstant().toEpochMilli();
        Date date = new Date(nextYearsTime);
        ...
        System.out.println("test whenUpdateSuccess:" + result);
    }

    @Test
    public void whenCreateUsernameRepeat() throws Exception {
        Date date = new Date();

        String content =
                "{\"username\":\"admin\", \"password\": null, \"birthday\": " + date.getTime() + "}";
        String result = mockMvc
                .perform(post("/user").contentType(MediaType.APPLICATION_JSON_UTF8).content(content))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value("1"))
